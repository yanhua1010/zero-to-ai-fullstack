...
把前面两块拼起来：
    PgVectorRetriever（检索）  →  prompts（拼 Prompt）  →  Claude API（生成）
...
- 没配 ANTHROPIC_API_KEY 时自动进入"本地降级模式"：
...
# 默认模型。注意：如果你用的是中转代理（设置了 ANTHROPIC_BASE_URL），
# 代理可能要求不同的模型名（例如 chatbot.py 里用的 "anthropic/claude-sonnet-4.6"）。
# 跑不通时第一件事就是把这个模型名改成你的代理认识的名字。
...
def _api_key_available() -> bool:
    ...
    placeholders = {"", "your_anthropic_api_key_here", "sk-ant-..."}
...
class RAGChain:
    ...
    def __init__(
        ...
        # 懒加载的 anthropic 客户端。没 Key 就一直是 None，走降级模式。
        self._client = None
        self.degraded = not _api_key_available()
        if self.degraded:
            logger.warning(
                "[RAGChain] 未检测到 ANTHROPIC_API_KEY，进入本地降级模式"
                "（只检索、不调用 LLM）"
            )
    ...
    def _generate(
        ...
        """
        调用 LLM 生成答案。messages 是完整的对话消息列表。

        降级模式下不调 API，返回 None，由调用方决定怎么兜底。
        """
        ...
        # anthropic SDK 的返回：content 是一个 block 列表，文本在第一个 block 的 .text
    ...
    def _degraded_answer(self, question: str, chunks: list[dict[str, Any]]) -> str:
        ...
        return (
            "（本地降级模式：未配置 ANTHROPIC_API_KEY，以下为检索到的原文，未经 LLM 总结）\n\n"
